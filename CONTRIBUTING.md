...
Full development and release validation requires `ext-event`, `ext-pcntl`, and `ext-posix`. Runwire keeps ext-event optional for production consumers, but the release-analysis toolchain must load it so PHPProbe can resolve the native `Event` / `EventBase` symbols used by the accelerated loop. Verify the release-analysis runtime before running the guard:

```bash
php -m | grep -E '^(event|pcntl|posix)
...
Portable-runtime acceptance remains intentionally separate and runs production autoloading without development requirements. Do not use `--ignore-platform-req=ext-event` to claim a release-guard pass.
