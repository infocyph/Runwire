...
The footprint evidence does not certify no slowdown. Final performance acceptance still requires a matched baseline/candidate comparison on the same stable infrastructure, PHP build, extensions, OPcache/JIT settings, Composer mode and workload, using the documented warm-up, five measured trials and applicable soak.
