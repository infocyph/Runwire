...
final class SwooleDriver implements HostDriverInterface
{
    ...
    private static function nativeServerFactory(): Closure
    {
        return static function (string $host, int $port): object {
            ...
            $server = $reflection->newInstance($host, $port);
            if (!is_object($server)) {
                throw new RuntimeUnavailableException('Unable to construct the Swoole/OpenSwoole HTTP server.');
            }
    ...
    {
        foreach (['OpenSwoole\\Http\\Server', 'Swoole\\Http\\Server'] as $class) {
