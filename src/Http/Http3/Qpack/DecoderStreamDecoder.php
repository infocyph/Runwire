...
final class DecoderStreamDecoder
{
    ...
    public function push(string $bytes): array
    {
        ...
        if (strlen($this->buffer) > $this->maxBufferedBytes) {
            throw new Http3Exception(ErrorCode::QPACK_DECODER_STREAM_ERROR, 'QPACK decoder stream buffer exceeds configured limit.');
        }
        $instructions = [];
        $offset = 0;
    ...
    private function decodeInstruction(int $offset): ?array
    {
        ...
        if (($first & 0x80) !== 0) { return $this->decode($offset, 7, DecoderInstructionType::SECTION_ACKNOWLEDGEMENT); }
        if (($first & 0x40) !== 0) { return $this->decode($offset, 6, DecoderInstructionType::STREAM_CANCELLATION); }
        $decoded = $this->decode($offset, 6, DecoderInstructionType::INSERT_COUNT_INCREMENT);
        if ($decoded !== null && $decoded[0]->value === 0) {
            throw new Http3Exception(ErrorCode::QPACK_DECODER_STREAM_ERROR, 'QPACK insert count increment cannot be zero.');
        }
        return $decoded;
    }
    ...
    {
        $decoded = IntegerCodec::tryDecode($this->buffer, $offset, $prefixBits, ErrorCode::QPACK_DECODER_STREAM_ERROR);
