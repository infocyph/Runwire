...
final class EncoderStreamDecoder
{
    ...
    public function bufferedBytes(): int { return strlen($this->buffer); }

    public function push(string $bytes): int
    {
        $this->buffer .= $bytes;
        if (strlen($this->buffer) > $this->maxBufferedBytes) {
            throw new Http3Exception(ErrorCode::QPACK_ENCODER_STREAM_ERROR, 'QPACK encoder stream buffer exceeds configured limit.');
        }
        $offset = 0;
        $insertions = 0;
        ...
        if ($offset > 0) { $this->buffer = substr($this->buffer, $offset); }
        return $insertions;
    }
    ...
    {
        if (!isset($this->buffer[$offset])) { return null; }
    ...
    {
        $decoded = IntegerCodec::tryDecode($this->buffer, $offset, 5, ErrorCode::QPACK_ENCODER_STREAM_ERROR);
        if ($decoded === null) { return null; }
        [$relative, $next] = $decoded;
        $entry = $this->table->entry($this->table->relativeToInsertCount($relative), ErrorCode::QPACK_ENCODER_STREAM_ERROR);
    ...
    {
        $name = StringCodec::tryDecode($this->buffer, $offset, 5, 0x20, $this->maxLiteralBytes, ErrorCode::QPACK_ENCODER_STREAM_ERROR);
        if ($name === null) { return null; }
        [$nameValue, $next] = $name;
        $value = StringCodec::tryDecode($this->buffer, $next, 7, 0x80, $this->maxLiteralBytes, ErrorCode::QPACK_ENCODER_STREAM_ERROR);
    ...
    private function insertNameReference(int $offset): ?array
    {
        ...
        $decoded = IntegerCodec::tryDecode($this->buffer, $offset, 6, ErrorCode::QPACK_ENCODER_STREAM_ERROR);
        if ($decoded === null) { return null; }
        [$index, $next] = $decoded;
        $value = StringCodec::tryDecode($this->buffer, $next, 7, 0x80, $this->maxLiteralBytes, ErrorCode::QPACK_ENCODER_STREAM_ERROR);
        if ($value === null) { return null; }
        [$fieldValue, $next] = $value;
        $name = $static ? StaticTable::entry($index, ErrorCode::QPACK_ENCODER_STREAM_ERROR)[0] : $this->table->entry($this->table->relativeToInsertCount($index), ErrorCode::QPACK_ENCODER_STREAM_ERROR)['name'];
