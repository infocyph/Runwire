...
final class Decoder
{
    ...
    public function __construct(
        ...
        if ($maxBlockedStreams < 0 || $maxBlockedBytes < 0) { throw new \InvalidArgumentException('QPACK decoder blocked-stream limits cannot be negative.'); }
    ...
    {
        foreach ($this->blocked[$streamId] ?? [] as $block) { $this->blockedBytes -= strlen($block); }
    ...
    {
        try { $section = $this->fieldDecoder->decode($block); }
        catch (BlockedFieldSectionException) { $this->block($block, $streamId); return null; }
    ...
    public function pushEncoderInstructions(string $bytes): array
    {
        ...
        if ($insertions > 0) { $this->pendingDecoderInstructions .= $this->decoderStream->insertCountIncrement($insertions); }
    ...
    {
        if ($section->requiredInsertCount > 0) { $this->pendingDecoderInstructions .= $this->decoderStream->sectionAcknowledgement($streamId); }
    }

    private function block(string $block, int $streamId): void
    {
        $newStream = !isset($this->blocked[$streamId]);
        if ($newStream && count($this->blocked) >= $this->maxBlockedStreams) { throw new Http3Exception(ErrorCode::QPACK_DECOMPRESSION_FAILED, 'Peer exceeded advertised QPACK blocked-stream limit.'); }
        if (strlen($block) > $this->maxBlockedBytes - $this->blockedBytes) { throw new Http3Exception(ErrorCode::EXCESSIVE_LOAD, 'Blocked QPACK field sections exceed configured memory limit.'); }
    ...
    private function retryBlocked(): array
    {
        ...
        foreach (array_keys($this->blocked) as $streamId) {
            while (($block = $this->blocked[$streamId][0] ?? null) !== null) {
                try { $section = $this->fieldDecoder->decode($block); }
                catch (BlockedFieldSectionException) { break; }
                array_shift($this->blocked[$streamId]);
                $this->blockedBytes -= strlen($block);
                $this->acknowledge($section, $streamId);
                $ready[] = new ReadyFieldSection($streamId, $section);
            }
            if (($this->blocked[$streamId] ?? []) === []) { unset($this->blocked[$streamId]); }
        }
        return $ready;
    }
}
