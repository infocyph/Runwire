...
/**
 * Implements a scalable ext-event loop for native workers.
 */
final class EventLoop implements LoopDiagnosticsProviderInterface, LoopInterface
{
    ...
    public function diagnostics(): LoopDiagnosticsSnapshot
    {
        return new LoopDiagnosticsSnapshot(
            ...
            timersActive: count($this->events) - count($this->readIndex) - count($this->writeIndex),
            deferredBacklog: 0,
    ...
    {
        return $this->watch($stream, $callback, Event::READ, $this->readIndex);
    }

    public function onWritable(mixed $stream, callable $callback): int
    {
        return $this->watch($stream, $callback, Event::WRITE, $this->writeIndex);
    }
    ...
    private function recordTick(int $started): void
    {
        $elapsed = max(0, MonotonicTime::nowNanoseconds() - $started);
        $this->lastTickNanoseconds = $elapsed;
        $this->maxTickNanoseconds = max($this->maxTickNanoseconds, $elapsed);
    }
    ...
    {
        $this->readIndex = array_filter($this->readIndex, static fn(int $value): bool => $value !== $id);
        $this->writeIndex = array_filter($this->writeIndex, static fn(int $value): bool => $value !== $id);
    }
    ...
    private function timer(float $seconds, callable $callback, bool $repeat): int
    {
        ...
        $event = Event::timer($this->base, function () use ($id, $closure, $repeat, $seconds, &$event, $deadline): void {
            if (!isset($this->events[$id])) {
                return;
            }

            $lag = max(0, MonotonicTime::nowNanoseconds() - $deadline);
            $this->maxLagNanoseconds = max($this->maxLagNanoseconds, $lag);
            if (!$repeat) {
                unset($this->events[$id]);
            }
        ...
        if (!$event instanceof Event || !$event->add($seconds)) {
    ...
    private function watch(mixed $stream, callable $callback, int $flag, array &$index): int
    {
        ...
        if (isset($index[$resourceId])) {
            throw new InvalidArgumentException(sprintf(
                ...
                $resourceId,
                $flag === Event::READ ? 'readable' : 'writable',
            ));
        }
